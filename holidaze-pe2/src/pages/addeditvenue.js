        ...
        <div className="container">
            <div className="row mb-5">
                ...
                <div className="col-sm-10">
                    ...
                    <p className="text-center mt-1 mb-4" id="userFeedback">Enter venue information below</p>
...
function UpdateExistingVenue(name,description, image, price, maxGuests, UpdateVenue){
    return (
        <div className="container">
            <div className="row mb-5">
                <div className="col"></div>
...
async function VenueUpdate(event, token, key, id, navigate, username){
    ...
    const name = event.target[0].value.length>0?event.target[0].value:event.target[0].placeholder;
    const description = event.target[1].value.length>0?event.target[1].value:event.target[1].placeholder;
    const image = event.target[2].value.length>0?event.target[2].value:event.target[2].placeholder;
    const price = event.target[3].value.length>0?Number(event.target[3].value):Number(event.target[3].placeholder);
    const maxGuests = event.target[4].value.length>0?Number(event.target[4].value):Number(event.target[4].placeholder);
...
async function VenueCreate(event, token, key, navigate, username){
    ...
    const image = event.target[2].value.length>0? event.target[2].value: "https://saterdesign.com/cdn/shop/products/property-placeholder_a9ec7710-1f1e-4654-9893-28c34e3b6399_2000x.jpg?v=1500393334";
...
function GetVenueInfo(id, setName, setDescription, setImage,setPrice, setMaxGuests){
    ...
    useEffect(() => {
        async function getData() {
            ...
            if(response.ok){
                ...
                json.data.media.length>0?setImage(json.data.media[0].url):setImage("");
...
function AddEditVenue(){
    ...
    if(loggedIn){
        if(params.id==="new"){
            ...
            GetVenueInfo(params.id, setName, setDescription, setImage,setPrice, setMaxGuests);
            addeditvenue = UpdateExistingVenue(name, description, image, price, maxGuests, UpdateVenue);
        }
    }
    else{
